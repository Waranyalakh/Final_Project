...
public class CalibrationData
{
    ...
    public void Tick(Quaternion targetRotation, float speed)
    {
        if (Mathf.Approximately(speed, 0f)) return; // ป้องกัน speed = 0
        if (Quaternion.Angle(initialRotation, targetRotation) < 0.0001f) return; // ป้องกัน Lerp กับค่าที่ต่างกันน้อยมาก
